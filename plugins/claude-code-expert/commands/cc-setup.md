...
## Phase 11: Sub-Repository Discovery & Setup

Scan for nested git repositories and ensure each has proper Claude Code configuration.

### 11.1 Sub-Repo Detection

```bash
# Find all .git directories within the project (excluding node_modules, vendor, etc.)
SUB_REPOS=$(find . -name ".git" -type d \
  ! -path "*/node_modules/*" \
  ! -path "*/.git/modules/*" \
  ! -path "*/vendor/*" \
  ! -path "*/.pnpm/*" \
...
# For monorepos, also check packages/, apps/, services/ for independent projects
MONO_DIRS=$(find ./packages ./apps ./services ./libs -maxdepth 1 -mindepth 1 -type d 2>/dev/null)
```

### 11.2 Per-Sub-Repo Setup

For each discovered sub-repo or monorepo package:

```bash
for repo in $SUB_REPOS $MONO_DIRS; do
  # Create .claude directory if missing
  mkdir -p "$repo/.claude/rules"

  # Create CLAUDE.md if missing (inherit from parent with overrides)
  if [ ! -f "$repo/CLAUDE.md" ]; then
    generate_sub_repo_claudemd "$repo"
  fi

  # Create minimal rules
  if [ ! -f "$repo/.claude/rules/lessons-learned.md" ]; then
    create_lessons_learned_template "$repo/.claude/rules/lessons-learned.md"
  fi

  # Create settings.json with inherited permissions
  if [ ! -f "$repo/.claude/settings.json" ]; then
    inherit_settings "$repo/.claude/settings.json"
  fi
done
```

### 11.3 Sub-Repo CLAUDE.md Template

```markdown
# {sub-repo-name} — Claude Code Instructions

## Parent Project
This is a sub-directory of the root project. See @../../CLAUDE.md for global conventions.

## Local Build Commands
{auto-detected from local package.json or build files}

## Scope
This package handles: {auto-detected from package.json description or README}

## Local Conventions
- Inherit all rules from @../../.claude/rules/
- Local overrides: (none by default)
```

---

## Phase 12: docs/context/ Scaffolding

Generate the documentation context structure for comprehensive project understanding.

### 12.1 docs/context/ Directory Structure

```
docs/
  context/
    project-overview.md        # What this system is, for whom
    architecture.md            # Top-level architecture diagram and narrative
    architecture-runtime.md    # Runtime view (calls, queues, external services)
    ...
    api-contracts.md           # Important endpoints, request/response
    api-guidelines.md          # REST/GraphQL patterns, pagination, errors
    testing-strategy.md        # How tests are organized
    security-rules.md          # AuthZ rules, PII handling, tenant isolation
    constraints.md             # Platform support, tech choices, SLAs
    ...
    plan.md                    # Current work plan (Claude maintains this)
    decisions/                 # Architecture Decision Records
      adr-template.md          # Template for new ADRs
```

### 12.2 Template Generation

Each file is generated with a header template and auto-populated sections based on
detected stack:

```markdown
# Project Overview

<!-- Auto-generated by /cc-setup. Fill in details as the project evolves. -->
<!-- Claude uses this file for context before making large changes. -->

## System Purpose
{Auto: from package.json description or README first paragraph}

## Tech Stack
{Auto: from Phase 1 detection}

## Key Stakeholders
- [ ] Fill in team members and their areas

## Non-Goals
- [ ] List what this system is NOT trying to do
```

### 12.3 CLAUDE.md Integration

After generating docs/context/, update the root CLAUDE.md to reference them:
...
## Phase 13: Fingerprint System

Create a fingerprint tracking what the plugin manages.

### 13.1 Fingerprint File
...
// .claude/fingerprint.json
{
  ...
  "pluginVersion": "5.0.0",
  "setupDate": "2026-03-19T00:00:00Z",
  ...
  "managedFiles": {
    "rules": [".claude/rules/architecture.md", ".claude/rules/lessons-learned.md"],
    "hooks": [".claude/hooks/lessons-learned-capture.sh", ".claude/hooks/anchor-state.sh"],
    "settings": [".claude/settings.json"],
    "docs": ["docs/context/project-overview.md", "docs/context/architecture.md"]
  },
  "subRepos": [
...
### 13.2 Fingerprint Usage

On `/cc-setup` or `/cc-update`:
1. Read existing fingerprint
2. Compare plugin version — if newer, trigger update
3. Compare stack detection — if changed, offer to regenerate configs
4. Compare managed files — only update files the plugin manages
5. Update fingerprint with new state
...
13. **Discover sub-repos** — Find nested git repos, ensure .claude setup (Phase 11)
14. **Scaffold docs/context** — Generate documentation structure (Phase 12)
15. **Fingerprint** — Save project state for incremental updates (Phase 13)
...
| `--sub-repos` | Discover and set up all nested repositories |
| `--docs` | Only generate docs/context/ scaffolding |
| `--fingerprint` | Only update fingerprint.json |
| `--update` | Incremental update using fingerprint (same as `/cc-update`) |
